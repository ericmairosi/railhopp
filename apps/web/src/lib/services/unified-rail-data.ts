...
export class UnifiedRailDataService {
   ...
   */
  async getEnhancedStationBoard(request: UnifiedStationBoardRequest): Promise<EnhancedStationBoard> {
    const crs = request.crs.toUpperCase();
    
    // Always get Darwin data first (as per user preference)
    const darwinBoard = await this.darwinClient.getDepartureBoard({
      crs,
    ...
    let enhancedBoard: EnhancedStationBoard = {
      ...darwinBoard,
      dataSource: 'darwin',
    ...
    if (knowledgeStationAvailable) {
      try {
        ...
        if (disruptionsResult.status === 'fulfilled' && disruptionsResult.value) {
          ...
          const relevantDisruptions = disruptionsResult.value.filter(disruption => 
            disruption.impact.routes.some(route => 
   ...
   * Uses Darwin as primary source, enriches with Knowledge Station data
   */
  async getEnhancedServiceDetails(request: UnifiedServiceDetailsRequest): Promise<EnhancedServiceDetails> {
    // Always get Darwin data first (as per user preference)
    const darwinDetails = await this.darwinClient.getServiceDetails({
      serviceId: request.serviceId
    });

    const knowledgeStationAvailable = this.knowledgeStationClient.isEnabled();
    let enhancedDetails: EnhancedServiceDetails = {
      ...darwinDetails,
      dataSource: 'darwin',
      knowledgeStationAvailable
    };
  ...
  async getStationInfo(crs: string): Promise<EnhancedStationInfo | null> {
    ...
    try {
      const board = await this.darwinClient.getDepartureBoard({
        crs: normalizedCrs,
        numRows: 1
      });
   ...
   */
  async getRelevantDisruptions(options: {
    crs?: string;
    serviceIds?: string[];
    ...
    try {
      ...
      return allDisruptions.filter(disruption => {
        ...
        if (options.crs) {
          const affectsStation = disruption.impact.routes.some(route =>
            route.origin === options.crs || route.destination === options.crs
          );
        ...
        if (!options.crs && !options.serviceIds) {
          return disruption.impact.operators.length === 0 || // Network-wide
                 disruption.severity === 'high'; // High severity always relevant
        }
    ...
    try {
      ...
      const baseServiceInfo: EnhancedServiceInfo = {
        darwinData: {
          ...
          scheduledDeparture: darwinDetails.previousCallingPoints[0]?.scheduledTime || '',
          estimatedDeparture: darwinDetails.previousCallingPoints[0]?.estimatedTime,
          platform: darwinDetails.platform,
          status: darwinDetails.status
        },
        dataSource: 'darwin',
