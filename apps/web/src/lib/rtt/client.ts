...
export class RTTClient {
  ...
  private async makeRequest(endpoint: string, options: RequestInit = {}): Promise<any> {
    if (!this.isEnabled()) {
      throw new RTTAPIError('RTT.io is not enabled or properly configured', 'NOT_ENABLED', {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new RTTAPIError(`HTTP ${response.status}: ${response.statusText}`, 'HTTP_ERROR', {
        ...
        if (!contentType.includes('application/json')) {
          throw new RTTAPIError('Expected JSON response from RTT.io API', 'INVALID_RESPONSE_TYPE', {
            contentType,
            url,
          })
        }
  ...
  async getDepartures(request: RTTDeparturesRequest): Promise<EnhancedDeparture[]> {
    ...
    try {
      ...
      throw new RTTAPIError('Failed to fetch departures from RTT.io', 'API_REQUEST_ERROR', error)
    }
  }
  ...
  async getStationInfo(crs: string): Promise<any> {
    try {
      ...
      throw new RTTAPIError('Failed to fetch station info from RTT.io', 'API_REQUEST_ERROR', error)
    }
  }
  ...
  private transformDepartures(response: RTTDeparturesResponse): EnhancedDeparture[] {
    ...
    return services.map((service) => ({
      ...
      scheduledDeparture:
        service.locationDetail.gbttBookedDeparture || service.locationDetail.publicDeparture || '',
      estimatedDeparture: service.locationDetail.realtimeDeparture || undefined,
      actualDeparture: service.locationDetail.actualDeparture || undefined,
      destination: {
      ...
      cancelReason: service.locationDetail.cancelReasonLongText || undefined,
      formation: service.formation
        ? {
            coaches: Number((service as any).formation.coaches) || 0,
            length: Number((service as any).formation.coaches) || 0,
          }
        : undefined,
...
export function getRTTClient(): RTTClient {
  if (!rttClient) {
    ...
    const enabled =
      process.env.RTT_ENABLED !== 'false' && process.env.KNOWLEDGE_STATION_ENABLED !== 'false'
